...
class MercadoLivreProcessing:

    def __init__(self):
        self.data = pd.read_json('../../raw_data/mercadolivre_products.json') 
    ...
    def origin_data(self):
        self.data['origin'] = 'Mercado Livre'
    ...
    def save(self):
        ...
        con = sqlite3.connect(f'{output_dir}/mercado_livre.db')
        self.data.to_sql('products', con, if_exists='replace', index=False)
        con.close()
        
        
if __name__ == '__main__':
    mercado_livre_processing = MercadoLivreProcessing()
    mercado_livre_processing.process()